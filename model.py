...
class CNNTransformer(nn.Module):
    def __init__(self,
                 logdir,
                 random_seed=0,
                 device="cuda:0",  # other options for device are e.g. "cuda:0" or "cpu"
        ...
        self.encoder = nn.TransformerEncoderLayer(
            d_model=filter_numbers[-1], nhead=attention_heads, dim_feedforward=hidden_units, dropout=dropout)
        self.linear = nn.Linear(filter_numbers[-1], 1)

    def forward(self, x):  # x has dimension (N,T)
        N, T = x.shape
        out = x.reshape((N, 1, T))  # (N,1,T)
        if self.use_convolution:
            for i in range(len(self.filter_numbers)-1):
                # (N,C,T), C is the number of channels/features
                out = self.convBlocks[i](out)
        out = out.permute(2, 0, 1)
        out = self.encoder(out)  # the input of the transformer is (T,N,C)
